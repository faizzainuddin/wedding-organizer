...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['name']) && isset($_POST['price']) && isset($_POST['qty'])) {
        ...
        // Panggil fungsi goodsEdit
        if (goodsEdit($id, $name, $price, $qty, $package_id)) {
            $message = "Barang berhasil diupdate!";
            $alert_type = "success";
        } else {
            $alert_type = "warning";
            $message = "Oops! Gagal mengupdate barang...";
        }
                            ...
                            <div class="container">
                                <form action="<?php echo $_SERVER["PHP_SELF"] . "?id=" . $id; ?>" method="POST" enctype="multipart/form-data">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="name" name="name" value="<?php echo htmlspecialchars($goods['g_name']); ?>" required>
                                    </div>
                                    <div class="row">
                                        <div class="mb-3 col-md-6">
                                            <label for="price" class="form-label">Harga</label>
                                            <input type="number" class="form-control" id="price" name="price" value="<?php echo htmlspecialchars($goods['g_price']); ?>" required>
                                        </div>
                                        <div class="mb-3 col-md-6">
                                            <label for="qty" class="form-label">Quantity</label>
                                            <input type="number" class="form-control" id="qty" name="qty" value="<?php echo htmlspecialchars($goods['g_qty']); ?>" required>
                                        </div>
                                    </div>
                                    <div class="mb-3">
                                        <label for="package_id" class="form-label">Pilih Paket</label><br>
                                        <small>*Pilih untuk ditambahkan kedalam paket</small>
                                        <select class="form-control" id="package_id" name="package_id" required>
                                            <?php foreach ($packages as $pkg) { ?>
                                                <option value="<?php echo $pkg['p_id']; ?>" <?php if ($pkg['p_id'] == $goods['g_p_id']) echo 'selected'; ?>><?php echo $pkg['p_name']; ?></option>
                                            <?php } ?>
                                        </select>
                                    </div>
